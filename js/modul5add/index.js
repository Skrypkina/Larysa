...
//     const summAllNumbers = (...args) => args.reduce((acc, arg) => {
//                 return acc += arg;
//     }, 0);
...
//   const setGuestState = (guests, period) => guests.map(guest => ({...guest,
...
// const getActiveGuests = arr => arr.filter(value => value.isActive);
...
// const getGuestById = (arr, id) => arr.find(value => value.id === id);
...
//   const ordered = Object.values(order).reduce((acc, value) => acc + value);
   
//   console.log(ordered); // 150
...
//   const getTotalPrice = (products, order) => Object.keys(order).reduce((acc, key) => {
//    return acc + (order[key] * products[key]);
...
//     const allGuestsActive = arr => arr.every(elem => elem.isActive);
