 ...
 */
const getAllNames = arr => arr.map(value => value.name);
 ...
 */
const getUsersByEyeColor = (arr, color) => arr.filter(value => {
    if (value.eyeColor === color) {
        return value;
    }
});
...
//  */
const getUsersByGender = (arr, gender) => arr.filter(value => value.gender === gender).map(key => key.name);
...
//  */
const getInactiveUsers = arr => arr.filter(value => !value.isActive);
...
//  */
const getUserByEmail = (arr, email) => arr.find(value => value.email === email);
...
//  */
const getUsersWithAge = (arr, min, max) => arr.filter(value => value.age > min && value.age < max);
...
//  */
const getUsersByFriend = (arr, name) => arr.filter(value => {
    if ((value.friends).includes(name))
        return value;
 ...
 */
const getAllSkills = arr => arr.reduce((acc, value,) => acc.concat(value.skills),[])
